...
const FOForm: React.FC<FOFormProps> = ({ values, setValues, options, autoPredict = false, onAutoPredictComplete }) => {
  ...
  const validateForm = (): boolean => {
    ...
    Object.entries(data).forEach(([key, val]) => {
      if (
        !val && val !== 0 && // empty (treat 0 as filled)
    ...
    const percentFields: Array<keyof FOFormValues> = ["percentage_primary_category", "allocation_deal_size_percentage_category", "allocation_percentage_category"];
