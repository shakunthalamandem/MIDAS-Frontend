...
interface FOSentimentAnalysisDumpDailyProps {
...
const fetchIpoTickers = async (
  ...
  const res = await fetch(`${apiUrl}/api/unified_writeup_details/`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      Authorization: token ? `Bearer ${token}` : "",
    },
    body: JSON.stringify({ deal_type: "FO", flag_for_writeup: "Y" }),
  });
...
const askPerplexity = async (question: string, uniqueDealId: string): Promise<Block[]> => {
  const res = await fetch(`${apiUrl}/api/fo_sentiment_perplexity_chat/`, {
    method: "POST",
...
const FOSentimentAnalysisDumpDaily: React.FC<FOSentimentAnalysisDumpDailyProps> = ({ focusTicker }) => {
    ...
    const loadTickers = async () => {
      ...
      try {
        ...
        const prepared = deals.map((deal) => ({
          ...deal,
          prompt: buildPrompt(deal.ticker, deal.deal_type, deal.issuer_name, deal.region, deal.fo_type, deal.listing_status),
...
export default FOSentimentAnalysisDumpDaily;
