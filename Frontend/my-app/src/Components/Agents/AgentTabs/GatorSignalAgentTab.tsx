...
interface GatorSignalAgentTabProps {
  ticker?: string;
}
...
const normalizeBlocks = (val: any): Block[] => {
  ...
  if (val && typeof val === "object") {
    ...
    if (Array.isArray((val as any).signal)) return (val as any).signal as Block[];
  }
...
const GatorSignalAgentTab: React.FC<GatorSignalAgentTabProps> = ({ ticker }) => {
  const [blocks, setBlocks] = useState<Block[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    const fetchGatorSignalData = async () => {
      if (!ticker) return;
      ...
      try {
        ...
        const response = await fetch(
          `${process.env.REACT_APP_API_URL}/api/summary_signal_board/`,
          {
            method: "POST",
            headers: {
        ...
        console.error("Error fetching gator signal data:", err);
        setError(err.message || "Failed to load gator signal analysis");
      } finally {
        setLoading(false);
      }
    };

    fetchGatorSignalData();
  }, [ticker]);
...
export default GatorSignalAgentTab;
