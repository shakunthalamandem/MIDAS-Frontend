...
const DetailedFundTable: React.FC = () => {
  ...
  const collapsedOnlyAssets = ["Cash", "Warrants", "Futures"]; // Optional logic
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // Sort months naturally (Jan–Dec, YTD last)
  ...
  {(() => {
    ...
    sortedFundNames.forEach((fundName) => {
      ...
      // Sum row under asset type column
      tableRows.push(
        <TableRow key={`${fundName}-sum`} sx={{ backgroundColor: "rgb(145, 206, 137)" }}>
          {/* No fund cell here because of rowSpan */}
