...
const COLUMNS: Array<{
  key: keyof SectorExposureRow;
  label: string;
  isCurrency: boolean;
  colorize: boolean;
}> = [
  { key: "sector", label: "SECTOR", isCurrency: false, colorize: false },
...
type SortKey = keyof SectorExposureRow;

interface Props {
  fund?: string;
}

const SectorExposurePnlBreakdown: React.FC<Props> = ({ fund }) => {
  const [rows, setRows] = useState<SectorExposureRow[]>([]);
  const [total, setTotal] = useState<SectorExposureRow | null>(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [sortKey, setSortKey] = useState<SortKey>("ytd_pnl");
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setError(null);
      try {
        const resp = await fetchSectorExposureBreakdown(fund);
        if (!cancelled) {
          setRows(resp.rows || []);
    ...
    <Paper elevation={1} sx={{ p: 3, borderRadius: 2 }}>
      <Box mb={2}>
        <Typography variant="subtitle1" sx={{ fontWeight: 700, color: "#1f3a8a" }}>
          Exposure & P&L Breakdown by Sector (ranked by YTD P&L)
        </Typography>
      </Box>
...
export default SectorExposurePnlBreakdown;
