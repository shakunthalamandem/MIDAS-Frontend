...
import { fetchDrawdown, DrawdownPoint } from "./monthlyDecApi";

interface Props {
  fund?: string;
}

const DrawdownChart: React.FC<Props> = ({ fund }) => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setError(null);
      try {
        const resp = await fetchDrawdown(fund);
    ...
    <Paper elevation={1} sx={{ p: 3, borderRadius: 2 }}>
      <Box mb={2}>
        <Typography variant="subtitle1" sx={{ fontWeight: 700, color: "#1f3a8a" }}>
          YTD Drawdown Depth Comparison
        </Typography>
        <Typography variant="caption" sx={{ color: "#6b7280" }}>
          HISTORICAL: DRAWDOWN (PORTFOLIO VS S&P)
        </Typography>
      </Box>
