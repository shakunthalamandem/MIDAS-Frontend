...
import {
  fetchMonthlyPnlByStrategy,
  MonthlyStrategyPnlResponse,
} from "./monthlyDecApi";
...
const MonthlyStrategyPnlTable: React.FC<Props> = ({ year, fund }) => {
  const [data, setData] = useState<MonthlyStrategyPnlResponse | null>(null);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setError(null);
      try {
        const resp = await fetchMonthlyPnlByStrategy(year, fund);
    ...
    <Paper elevation={1} sx={{ p: 3, borderRadius: 2 }}>
      <Box mb={2}>
        <Typography variant="subtitle1" sx={{ fontWeight: 700, color: "#1f3a8a" }}>
          Monthly P&L Contribution by Strategy Sleeve
        </Typography>
      </Box>
          ...
          <Table size="small">
            <TableHead>
              <TableRow sx={{ backgroundColor: HEADER_BG }}>
                <TableCell sx={{ color: "#fff", fontWeight: 700 }}>Asset Type</TableCell>
                {data.months.map((m) => (
            ...
            <TableBody>
              ...
              <TableRow sx={{ backgroundColor: "#d1d5db" }}>
                <TableCell sx={{ fontWeight: 700 }}>{data.total_row.asset_type}</TableCell>
                {data.months.map((m) => {
...
export default MonthlyStrategyPnlTable;
