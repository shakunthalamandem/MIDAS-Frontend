...
import { fetchPnlByDesk, DeskPnlRow } from "./monthlyDecApi";
...
const AnalystDeskPnlTable: React.FC<Props> = ({ fund }) => {
  const [rows, setRows] = useState<DeskPnlRow[]>([]);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setError(null);
      try {
        const resp = await fetchPnlByDesk(fund);
        if (!cancelled) {
          setRows(resp.data || []);
    ...
    <Paper elevation={1} sx={{ p: 3, borderRadius: 2, height: "100%" }}>
      <Box mb={2}>
        <Typography variant="subtitle1" sx={{ fontWeight: 700, color: "#1f3a8a" }}>
          YTD P&L Contribution by Analyst Desk
        </Typography>
      </Box>
...
export default AnalystDeskPnlTable;
