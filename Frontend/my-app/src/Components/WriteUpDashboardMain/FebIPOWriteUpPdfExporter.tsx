  ...
  const handleExport = async () => {
    ...
    try {
      ...
      if (ticker) {
        ...
        pdf.text(
          tickerText,
          pdfWidth - marginX - pdf.getTextWidth(tickerText),
          20,
        );
        ...
        if (issuerName?.trim()) {
          ...
          pdf.text(
            displayName,
            pdfWidth - marginX - pdf.getTextWidth(displayName),
            28,
          );
        }

        // Exchange and pricing date below company name
        const additionalParts = [exchange?.trim(), pricingDate?.trim()].filter(
          Boolean,
        );
        if (additionalParts.length > 0) {
          ...
          pdf.text(
            additionalText,
            pdfWidth - marginX - pdf.getTextWidth(additionalText),
            35,
          );
        }
      }
      ...
      root
        .querySelectorAll<HTMLElement>(
          ".MuiIconButton-root, .pdf-hidden, button[aria-label='add-item'], button[aria-label='delete-item']",
        )
        .forEach(hideForPdf);
      ...
      for (const section of targets) {
        ...
        const relaxLayout = (el: HTMLElement) => {
          ...
          children.forEach((child) => {
            ...
            if (
              child.tagName === "TABLE" ||
              child.classList.contains("MuiTable-root")
            ) {
        ...
        const canvas = await html2canvas(section, {
          ...
          onclone: (doc) => {
            ...
            if (cloned) {
              ...
              const accordions =
                cloned.querySelectorAll<HTMLElement>(".MuiAccordion-root");
              accordions.forEach((accordion) => {
                accordion.classList.add("Mui-expanded");
                const content =
                  accordion.querySelector<HTMLElement>(".MuiCollapse-root");
                if (content) {
              ...
              const riskMeterBars =
                cloned.querySelectorAll<HTMLElement>(".risk-meter-bar");
              ...
              const riskMeterBadges =
                cloned.querySelectorAll<HTMLElement>(".risk-meter-badge");
        ...
        while (offsetPx < canvas.height) {
          ...
          // Nothing fits → close page and start a new one
