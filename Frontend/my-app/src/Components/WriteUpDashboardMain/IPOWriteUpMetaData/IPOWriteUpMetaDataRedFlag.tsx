...
<<<<<<< HEAD
  ...
  const computeRating = (avg: number) => {
    if (avg >= 1 && avg < 2) return "Low Risk"
    if (avg >= 2 && avg < 3) return "Slight Risk"
    if (avg >= 3 && avg < 4) return "Moderate Risk"
    if (avg >= 4 && avg <= 5) return "Significant Risk"
    return "0"
  }
  ...
  const updateLocalItems = (updatedItems: RedFlagItem[]) => {
    ...
    const nextRating = computeRating(nextAvg)
    setAnalysis((prev) => ({
      ...prev,
      data: updatedItems,
      avg_score: nextAvg,
