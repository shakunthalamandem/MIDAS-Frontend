...
const S1DocumentUpload: React.FC = () => {
  ...
  const handleUpload = async () => {
    ...
    if (!tickerValue) {
      setUploadError("Please search and select a ticker.");
      return;
    }
    ...
    setUploading(true);
    try {
      const res = await fetch(`${apiUrl}/api/s1file_upload/`, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
        },
...
export default S1DocumentUpload;
