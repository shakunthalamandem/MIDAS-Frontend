...
const getPreferredContentOrientation = (
  ...
  const tables = Array.from(
    root.querySelectorAll<HTMLElement>("table, .MuiTable-root")
  );
...
const collectTableGuards = (section: HTMLElement): TableGuard[] => {
  ...
  return Array.from(
    section.querySelectorAll<HTMLElement>("table, .MuiTable-root")
  )
    .map((table) => {
...
const collectKeepTogetherGuards = (section: HTMLElement): KeepTogetherGuard[] => {
  ...
  const visualNodes = Array.from(
    section.querySelectorAll<HTMLElement>("canvas, svg, img")
  );
