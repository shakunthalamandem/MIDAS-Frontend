...
const FOCombined: React.FC<CombinedProps> = ({ shareData, tradingData, ticker }) => {
  ...
  const handleSave = async (type: "share" | "trading") => {
    try {
      ...
      const payload = { ticker, ...(type === "share" ? shareLocal : tradingLocal) };
      ...
      <Box display="flex" gap={3} flexWrap="wrap">
            ...
            {tradingFields.map((field) => (
              <Box key={field.key} flex="1 1 calc(50% - 12px)" minWidth="250px" mb={2}>
                ...
                {editTrading ? (
                  ...
                  <Typography sx={{ fontWeight: 400 ,fontSize:'18px',color:'#333333'}}>{formatValue(tradingLocal[field.key], field.suffix)}</Typography>
                )}
              </Box>
            ))}
          </Box>
        ...
        <motion.div
          ...
          style={{
            ...
            padding: "20px",
            position: "relative",
            minHeight: "400px", // same height for both cards
          }}
        >
          <IconButton
          ...
          <Box display="flex" flexWrap="wrap" gap={3} justifyContent="space-between">
              ...
              <Box key={field.key} flex="1 1 calc(50% - 12px)" minWidth="250px" mb={2}>
                  ...
                  {editShare ? (
                    ...
                    <Typography sx={{ fontWeight: 400 ,fontSize:'18px',color:'#333333'}}>{formatValue(shareLocal[field.key], "%")}</Typography>
                  )}
                </Box>
              ))}
