...
const GapDataTable: React.FC<GapDataTableProps> = ({ data }) => {
  ...
  const preprocessRows = (rows: any[]) =>
    rows.map((row, index) => ({
      id: index,
      ...row,
      deal_size: row.deal_size ? formatDealSize(row.deal_size.toFixed()) : "",

      allocation_return: row.allocation_return
        ? `$${row.allocation_return.toFixed()}`
        : "",
      allocation_ioi_percentage: row.allocation_ioi_percentage
        ? `${row.allocation_ioi_percentage.toFixed()}%`
        : "",
      model_am_return: row.model_am_return
        ? `${formatDealSize(row.model_am_return.toFixed())}`
        : "",
      model_am_capital: row.model_am_capital
        ? `${formatDealSize(row.model_am_capital.toFixed())}`
        : "",
      model_return_1_allocation: row.model_return_1_allocation
        ? `${formatDealSize(row.model_return_1_allocation.toFixed())}`
        : "",
      model_capital_1_allocation: row.model_capital_1_allocation
        ? `${formatDealSize(row.model_capital_1_allocation.toFixed())}`
        : "",
      model_actual_return: row.model_actual_return
        ? `${formatDealSize(row.model_actual_return.toFixed())}`
        : "",
      am_return: row.am_return
        ? `${formatDealSize(row.am_return.toFixed())}`
        : "",
      t1d_return_from_bloomberg: row.t1d_return_from_bloomberg
        ? `${row.t1d_return_from_bloomberg.toFixed(2)}%`
        : "",
      t1m_return_from_dealogic: row.t1m_return_from_dealogic
        ? `${row.t1m_return_from_dealogic.toFixed(2)}%`
        : "",
      total_committed_capital: row.total_committed_capital
        ? `${formatDealSize(row.total_committed_capital.toFixed())}`
        : "",
      am_capital_committed: row.am_capital_committed
        ? `${formatDealSize(row.am_capital_committed.toFixed())}`
        : "",
      allocated_capital: row.allocated_capital
        ? `${formatDealSize(row.allocated_capital.toFixed())}`
        : "",
      allocation_price: row.allocation_price
        ? `${row.allocation_price.toFixed()}%`
        : "",
      subscription_bid_shares: row.subscription_bid_shares
        ? `${row.subscription_bid_shares.toFixed(2)}%`
        : "",
      allocation_deal_size_percentage: row.allocation_deal_size_percentage
        ? `${row.allocation_deal_size_percentage.toFixed(2)}%`
        : "",
    }));
