...
const GapDataTable: React.FC<GapDataTableProps> = ({ data }) => {
  ...
  const preprocessRows = (rows: any[]) =>
    rows.map((row, index) => ({
      ...
      allocation_return: row.allocation_return ? `$${row.allocation_return.toFixed()}` : "",
      allocation_ioi_percentage: row.allocation_ioi_percentage ? `${row.allocation_ioi_percentage.toFixed()}%` : "",
