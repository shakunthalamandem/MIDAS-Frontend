...
const getMondayOfWeek = (week: number, year: number): string => {
  ...
  const highestWeek2026 = Object.keys(data["2026"] || {}).reduce((highest, week) => {
    const cumulativeCount2026 = data["2026"]?.[week]?.cumulative_count || 0;
...
<Tooltip
  content={({ active, payload, label }) => {
    if (active && payload && payload.length) {
      ...
      const monday = getMondayOfWeek(weekNum, 2026); 
