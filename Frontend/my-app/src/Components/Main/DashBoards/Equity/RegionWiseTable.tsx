...
const getMaxValues = (data: { region: string; data: RegionData }[]) => ({
  ...
  Long_Opportunity_Value: Math.max(...data.map(d => d.data.Long_Opportunity_Value)),
  Positively_Performing_Deals_Percentage: Math.max(...data.map(d => d.data.Positively_Performing_Deals_Percentage)),
  Expected_Returns_Excess: Math.max(...data.map(d => d.data.Expected_Returns_Excess)),
});
          ...
          <TableBody>
            ...
            {ipoData.map(row => (
              <TableRow key={`IPO-${row.region}`}>
                ...
                <TableCell sx={getStyle(row.data.Total_Deal_Count, ipoMax.Total_Deal_Count)}>{row.data.Total_Deal_Count}</TableCell>
                <TableCell sx={getStyle(row.data.Total_Deal_Volume, ipoMax.Total_Deal_Volume)}>{formatNumber(row.data.Total_Deal_Volume)}</TableCell>
                <TableCell sx={getStyle(row.data.Long_Opportunity_Value, ipoMax.Long_Opportunity_Value)}>{formatNumber(row.data.Long_Opportunity_Value)}</TableCell>
                <TableCell sx={getStyle(row.data.Positively_Performing_Deals_Percentage, ipoMax.Positively_Performing_Deals_Percentage)}>{row.data.Positively_Performing_Deals_Percentage}%</TableCell>
                <TableCell sx={getStyle(row.data.Expected_Returns_Excess, ipoMax.Expected_Returns_Excess)}>{row.data.Expected_Returns_Excess}%</TableCell>
              </TableRow>
            ))}
            <TableRow>
              <TableCell colSpan={6} sx={{ fontWeight: 'bold', backgroundColor: '#002060', color: "white", textAlign: "center" }}>
            ...
            {foData.map(row => (
              <TableRow key={`FO-${row.region}`}>
                <TableCell>{row.region}</TableCell>
                <TableCell sx={getStyle(row.data.Total_Deal_Count, foMax.Total_Deal_Count)}>{row.data.Total_Deal_Count}</TableCell>
                <TableCell sx={getStyle(row.data.Total_Deal_Volume, foMax.Total_Deal_Volume)}>{formatNumber(row.data.Total_Deal_Volume)}</TableCell>
                <TableCell sx={getStyle(row.data.Long_Opportunity_Value, foMax.Long_Opportunity_Value)}>{formatNumber(row.data.Long_Opportunity_Value)}</TableCell>
                <TableCell sx={getStyle(row.data.Positively_Performing_Deals_Percentage, foMax.Positively_Performing_Deals_Percentage)}>{row.data.Positively_Performing_Deals_Percentage}%</TableCell>
                <TableCell sx={getStyle(row.data.Expected_Returns_Excess, foMax.Expected_Returns_Excess)}>{row.data.Expected_Returns_Excess}%</TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>
