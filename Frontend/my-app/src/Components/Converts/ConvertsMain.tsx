...
import ConvertsDealSearch from "./Tabs/ConvertsDealSearch";
import ConvertsDealStatsMain from "./Tabs/ConvertsDealStatsMain";
import ConvertsSkewMain from "./Tabs/ConvertsSkewMain";

const ConvertsMain: React.FC = () => {
  const [value, setValue] = useState<number>(0);
  const navigate = useNavigate(); 
  const { ticker: routeTicker } = useParams<{ ticker: string }>();
  const [searchTerm, setSearchTerm] = useState<string>("");
  const [loading, setLoading] = useState<boolean>(false);
  const [selectedTicker, setSelectedTicker] = useState<string>(routeTicker || "WisdomTree Inc"); 
...
useEffect(() => {
  ...
  const handleSearch = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`${apiUrl}/api/converts_search/${query}`, {
        method: "GET",
        headers: {
  ...
  return (
    <Box sx={{ width: "100%", backgroundColor: "#fff" }}>
      ...
      {value === 0 && selectedTicker && (
        <ConvertsDealSearch issuer_name={selectedTicker}  />
      )}
      {value === 1 && <ConvertsDealStatsMain />}
      {value === 2 && <ConvertsSkewMain />}
    </Box>
  );
};

export default ConvertsMain;
