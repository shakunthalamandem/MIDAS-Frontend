...
/* Keys that get rendered as summary cards (when present in response) */
...
/* Keys that get rendered as horizontal exposure bars */
...
const RiskTriggers: React.FC = () => {
  ...
  /* ── Summary cards (driven by response) ── */
                ...
                return (
                  ...
                  >
                    ...
                    <Box className="trig-exp-card-value-wrap">
                      <span className={`trig-exp-card-value ${isNegative ? "trig-exp-card-value--neg" : "trig-exp-card-value--pos"}`}>
                        {rawVal}
                      </span>
                    </Box>
                  </Box>
  ...
  /* ── Derive display groups dynamically from response ── */
          ...
          <Box className="trig-section">
            ...
            {Object.entries(data.limits || {}).map(([key, limitSection]) => (
              <Box key={key} className="trig-metric">
                <Typography className="trig-metric-name-standalone">{limitSection?.title || key}</Typography>
                <table className="trig-table">
                  <thead>
                    <tr>
                  ...
                  <tbody>
                    {!limitSection?.data || limitSection.data.length === 0 ? (
                      ...
                      limitSection.data.map((row, i) => {
                        ...
                        const threshold = data.current_levels?.top_10_issuer_delta_net_exposure?.guideline;
                        const status = getStatus(numVal, threshold);
                        return (
                          <tr key={i}>
